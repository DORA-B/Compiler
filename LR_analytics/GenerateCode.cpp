...
objectCodeGenerator::objectCodeGenerator(vector<Quadruple> ic, vector<blockItem> bg, int stack_size)
{
	...
	for (auto block_no = 0; block_no < block_group.size(); block_no++)
	{
		...
		for (auto i = 0; i < MessageTable.size(); i++)
		{
			...
			if (TAS.arg1 == "" || TAS.op == "=[]")
				...
				Reg_arg1 = STACK + "($sp)";
			else if (TAS.arg1.find_first_not_of("0123456789") == string::npos)
			{
				if (TAS.op == "+")
					Reg_arg1 = TAS.arg1;
				else
			...
			if (TAS.arg2 == "")
				...
				Reg_arg2 = STACK + "($sp)";
			else if (TAS.arg2.find_first_not_of("0123456789") == string::npos)
			{
				if (TAS.op == "+" && !(Reg_arg1.find_first_not_of("0123456789") == string::npos))//不能有两个立即数
					Reg_arg2 = TAS.arg2;
				else
				{
			...
			{
				//cerr << "ERROR: 非法的中间代码" << TAS.op << '\t' << TAS.arg1 << '\t' << TAS.arg2 << '\t' << TAS.res << "\n";
				//exit(-1);
...
void objectCodeGenerator::showMessageTableHistory()
{
	for (auto tno = 0; tno < messageTableHistory.size(); tno++)
	{
		...
		cout << "(" << message_table.no << ")\t" << message_table.TAS.op << ' ' << message_table.TAS.arg1 << ' ' << message_table.TAS.arg2 << ' ' << message_table.TAS.res;
...
void objectCodeGenerator::showAnalysisHistory()
{
	for (auto ino = 0; ino < analysisHistory.size(); ino++)
	{
		...
		cout << "\n****************(" << e.TAS.op << "," << e.TAS.arg1 << "," << e.TAS.arg2 << "," << e.TAS.res << ")****************\n";
