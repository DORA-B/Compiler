...
optimizerAnalysis::optimizerAnalysis(map<int, string> nt, symbolTable* gt, vector<Quadruple> ic)
{
	name_table = nt;
	global_table = gt;
	OptimiseQuadruple = ic;
}
...
bool optimizerAnalysis::preOptimize()
{
	...
	for (auto i = 0; i < OptimiseQuadruple.size(); i++)
	{
		Quadruple* e = &OptimiseQuadruple[i];
		if (e->op == "jal")
		{
...
void optimizerAnalysis::partition()
{
	...
	{
		Quadruple e = OptimiseQuadruple[i];
		bool jmp_flag = (i - 1 >= 0 && (OptimiseQuadruple[i - 1].op[0] == 'j' || OptimiseQuadruple[i - 1].op == "ret"));
		...
		bool enter_flag = ((i + 1 < OptimiseQuadruple.size() && (OptimiseQuadruple[i + 1].op[0] == 'L' || OptimiseQuadruple[i + 1].op[0] == 'F')) || e.op[0] == 'j' || e.op == "ret" || e.op == "break");
		if (enter_flag)
		{
			block.end = i;
	...
	for (auto pos = 0; pos < OptimiseQuadruple.size(); pos++)
	{
		if (OptimiseQuadruple[pos].op[0] == 'L')
			label_loc[OptimiseQuadruple[pos].op] = pos;
	}
	...
	for (auto i = 0; i < block_group.size(); i++)
	{
		...
		if (OptimiseQuadruple[e->end].op == "ret")
		{
		...
		else//这个基本块不发生返回
		{
			...
			int prepos = pos - 1;
			while (prepos < OptimiseQuadruple.size())
			{
				if (label_loc.find(OptimiseQuadruple[prepos].res) != label_loc.end()\
					&& label_loc[OptimiseQuadruple[prepos].res] < pos)
				{
					pos = label_loc[OptimiseQuadruple[prepos].res];
					prepos = label_loc[OptimiseQuadruple[prepos].res];
				}
				prepos++;
			}

			while (pos < OptimiseQuadruple.size() && OptimiseQuadruple[pos].op[0] != 'F')
			{
				if (find(e->wait_variable.begin(), e->wait_variable.end(), OptimiseQuadruple[pos].arg1) != e->wait_variable.end() && find(real_wait_variable.begin(), real_wait_variable.end(), OptimiseQuadruple[pos].arg1) == real_wait_variable.end())
				{
					real_wait_variable.push_back(OptimiseQuadruple[pos].arg1);
				}
				if (find(e->wait_variable.begin(), e->wait_variable.end(), OptimiseQuadruple[pos].arg2) != e->wait_variable.end() && find(real_wait_variable.begin(), real_wait_variable.end(), OptimiseQuadruple[pos].arg2) == real_wait_variable.end())
				{
					real_wait_variable.push_back(OptimiseQuadruple[pos].arg2);
				}
				pos++;
			}
...
vector<DAGitem> optimizerAnalysis::geneDAG(int block_no)
{
	...
	{
		string op = OptimiseQuadruple[pos].op;
		string B = OptimiseQuadruple[pos].arg1;
		string C = OptimiseQuadruple[pos].arg2;
		string A = OptimiseQuadruple[pos].res;
		int element_count;
		...
		if (element_count == -1)
		{
			...
			newDAG.code = OptimiseQuadruple[pos];
...
void optimizerAnalysis::optimize()
{
		...
		for (auto i = 0; i < DAG.size(); i++)
		{
			...
			else
			{
				if (DAG[i].isleaf)
				{
					for (auto j = 0; j < DAG[i].label.size(); j++)
					{
						...
						if (DAG[i].value[0] == '-')
							...
							v = DAG[i].value;
						Quadruple newTAS = { ":=",v,"",DAG[i].label[j] };
				...
				else
				{
					...
					{
						Quadruple newTAS = { DAG[i].op,lv,rv,DAG[i].label[0] };
						optimized_code.push_back(newTAS);
						for (auto label_no = 1; label_no < DAG[i].label.size(); label_no++)
						{
							Quadruple newTAS = { ":=",DAG[i].label[0],"",DAG[i].label[label_no] };
							optimized_code.push_back(newTAS);
						}
					}
				}
	...
	{
		Quadruple TAS = optimized_code[pos];
		...
		if ((TAS.res[0] == 'T' || TAS.res[0] == 'S') && tmpV_map.find(TAS.res) == tmpV_map.end())
			tmpV_map[TAS.res] = "T" + to_string(newtemp_counter++);
	}
	for (auto pos = 0; pos < optimized_code.size(); pos++)
	{
	...
	unoptimized_code = OptimiseQuadruple;
	block_group.clear();
	OptimiseQuadruple = optimized_code;
	partition();
}
...
void optimizerAnalysis::showIntermediateCode()
{
	...
	for (auto i = 0; i < OptimiseQuadruple.size(); i++)
	{
		cout << "(" << i << ")\t" << OptimiseQuadruple[i].op << '\t' << OptimiseQuadruple[i].arg1 << '\t' << OptimiseQuadruple[i].arg2 << '\t' << OptimiseQuadruple[i].res << endl;
	}
	cout << "*********************************\n";
}
...
void optimizerAnalysis::showDAG()
{
	for (auto no = 0; no < DAG_group.size(); no++)
	{
		...
		for (auto i = 0; i < DAG.size(); i++)
		{
			...
			if (DAG[i].isremain)
			{
				cout << DAG[i].code.op << '\t' << DAG[i].code.arg1 << '\t' << DAG[i].code.arg2 << '\t' << DAG[i].code.res << endl;
			}
			else
			{
...
double optimizerAnalysis::analysis()
{
	...
	//showBlockGroup();
	int route = 0;
	int original_size = OptimiseQuadruple.size();
	int optimize_size = 0;
	do
	{
		optimize();
		route++;
	} while (unoptimized_code.size() != OptimiseQuadruple.size());
	optimize_size = OptimiseQuadruple.size();
	cout << "一共优化了" << route << "轮\n";
